 ...
 */
/*:ja
 * @plugindesc v1.01 新しい制御文字を追加します
 * @author Victor Sant
 *
 * @help
 * 翻訳:ムノクラ
 ...
 *
 * 元プラグイン:
 * https://victorenginescripts.wordpress.com/rpg-maker-mv/escape-codes/
 *
 *
 * ===========================================================================
 * 必要プラグイン
 ...
 *
 * 制御文字は、\c[x], \n[x], \i[x]のようなコードです。
 * 文章の表示や他のいくつかのウィンドウで使用することができます。
 * 以下のコードを制御文字として使用することができます。
 *
 * 色コード:
 * \c[#rrggbb] : 16進数で色を変更 (00-FF)
 * \c[r,g,b]   : RGB値を使って色を変更 (0-255)
 *
 * ---------------
 *
 * フォントコード:
 ...
 * \fi    : イタリック体に切り替え
 * \fs    : 影を切り替え
 * \fo[x] : アウトラインの色を変更(16進数 / RGB値)
 * \fw[x] : アウトラインの幅を変更
 *
 * ---------------
 *
 * アクターコード:
 * \an[x]    : 二つ名を表示(actor ID)
 * \ac[x]    : 職業を表示(actor ID)
 * \al[x]    : レベルを表示(actor ID)
 * \ap[x, y] : 能力値を表示(actor ID, parameter ID)
 *
 * ---------------
 *
 * アイコンコード:
 * \ii[x] : アイテムのアイコンを表示(Item ID)
 * \iw[x] : 武器のアイコンを表示(Weapon ID)
 * \ia[x] : 防具のアイコンを表示(Armor ID)
 * \is[x] : スキルのアイコンを表示(Skill ID)
 * \it[x] : ステートのアイコンを表示(State ID)
 *
 * ---------------
 *
 * 名前コード:
 * \nc[x]  : 職業名を表示 (Class ID)
 * \ni[x]  : アイテム名を表示(Item ID)
 * \nw[x]  : 武器名を表示(Weapon ID)
 * \na[x]  : 防具名を表示 (Armor ID)
 * \ns[x]  : スキル名を表示(Skill ID)
 * \nt[x]  : ステート名を表示(State ID)
 * \np[x]  : 能力名を表示(Parameter ID)
 * \nsw[x] : スイッチ名を表示(Switch ID)
 * \nvr[x] : 変数名を表示(Variable ID)
 * \nev[x] : イベント名を表示(Event ID)
 * \nen[x] : 敵キャラ名を表示(Enemy ID)
 * \nel[x] : 属性名を表示(Element ID)
 * \nwt[x] : 武器タイプ名を表示(Weapon Type ID)
 * \nat[x] : 防具タイプ名を表示(Armor Type ID)
 * \nst[x] : スキルタイプ名を表示(Skill Type ID)
 * \nmp    : 現在のマップ名を表示
 *
 * ---------------
 *
 ...
 * \r  : フォントの設定をリセット
 * \tg : パーティが所持金の合計を表示
 * \fv[x]  : フロアフォーマット」を使用して変数値を表示
 *             変数が0以上の場合は表示 'FX'
 *               例: 15F
 *             変数が0よりも小さい場合は表示 'XB'
 *               例: B12
 * \e['x'] : コードは常に引用符で囲まれていなければなりません。
 *             例: \e['$gameSystem.playtimeText()']
 */
